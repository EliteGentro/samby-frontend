...
describe('BehavioralCollectionMatrix component', () => {
  it('renders the behavioral matrix with metrics and customer rows', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Behavioral Cash Collection Matrix' }),
    ).toBeInTheDocument()
    ...
    expect(screen.getByText('Desfase de liquidez vs \'Net 30\'')).toBeInTheDocument()

    // Customers from demo data
    expect(screen.getAllByText('Comercial Atlas').length).toBeGreaterThanOrEqual(1)
    expect(screen.getAllByText('Grupo Rivera').length).toBeGreaterThanOrEqual(1)
    expect(screen.getAllByText('Ferretería Central').length).toBeGreaterThanOrEqual(1)
  })
  ...
  it('activates ASEM stress toggle (+76 days) and reveals the PyME risk banner', () => {
    ...
    expect(
      screen.getByText(/Efecto ASEM de 76 días aplicado \(Simulación de Estrés PyME\)/i),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByText('Plazo con Estrés ASEM (+76d)'),
    ).toBeInTheDocument()
  ...
  it('switches between P50, P80 and Naive perspectives', () => {
    ...
    expect(
      screen.getByText(/Supuesto Ingenuo Net 30/i),
    ).toBeInTheDocument()
  })
