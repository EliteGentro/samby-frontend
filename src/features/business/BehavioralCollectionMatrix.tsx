...
import { money, number, type Page, type Workspace } from '../../domain/workspace'
...
export function BehavioralCollectionMatrix({
  ...
  const [showInvoiceDetails, setShowInvoiceDetails] = useState<boolean>(true)
    ...
    <Panel
      ...
      action={
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          >
            <ShieldAlert size={16} />
            {asemStress ? 'Estrés ASEM Activo (+76d)' : 'Activar Estrés ASEM (+76d)'}
          </button>
          {onNavigate && (
            <button
              type="button"
    ...
    >
      {asemStress && (
        ...
        >
          ...
          <div>
            <strong>Efecto ASEM de 76 días aplicado (Simulación de Estrés PyME)</strong>
            <p style={{ margin: '0.25rem 0 0 0' }}>
              De acuerdo con la <em>Radiografía del Emprendimiento de la ASEM</em> (Asociación de Emprendedores de México),
              las PyMEs mexicanas sufren demoras de pago que promedian <strong>+76 días de retraso adicional</strong> sobre los plazos pactados.
              Esta simulación desplaza los cobros esperados hacia horizontes tardíos para revelar la brecha oculta de liquidez.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="metrics-grid">
        ...
        <MetricCard
          capability="internal-debt"
          label={asemStress ? 'Plazo con Estrés ASEM (+76d)' : 'Plazo real empírico (P50)'}
      ...
      <section aria-label="Matriz Conductual de Cobro">
        ...
        >
          <div>
            <h3 style={{ margin: 0 }}>Matriz Conductual de Distribución de Cobro</h3>
            <p className="muted small" style={{ margin: '0.25rem 0 0 0' }}>
              Muestra en qué ventanas temporales ingresará realmente el efectivo según el{' '}
              <strong>
                {asemStress
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th scope="col" style={{ textAlign: 'right' }}>91–120 días</th>
                  <th scope="col" style={{ textAlign: 'right' }}>120+ días</th>
                ...
                {result.matrixRows.map((row) => (
                  <tr key={row.customer}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <span className={row.buckets['0-30'] > 0 ? 'badge green' : 'muted'}>
                        {row.buckets['0-30'] > 0 ? money(row.buckets['0-30'], currency) : '—'}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <span className={row.buckets['31-60'] > 0 ? 'badge' : 'muted'}>
                        {row.buckets['31-60'] > 0 ? money(row.buckets['31-60'], currency) : '—'}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <span className={row.buckets['61-90'] > 0 ? 'badge amber' : 'muted'}>
                        {row.buckets['61-90'] > 0 ? money(row.buckets['61-90'], currency) : '—'}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <span className={row.buckets['91-120'] > 0 ? 'badge amber' : 'muted'}>
                        {row.buckets['91-120'] > 0 ? money(row.buckets['91-120'], currency) : '—'}
                      </span>
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <span className={row.buckets['120+'] > 0 ? 'badge red' : 'muted'}>
                        {row.buckets['120+'] > 0 ? money(row.buckets['120+'], currency) : '—'}
                      </span>
                    </td>
                    <td>
                      <span className={`badge ${row.profile.p50DelayDays <= 0 ? 'green' : row.profile.p50DelayDays <= 15 ? '' : 'amber'}`}>
              ...
              <tfoot>
                <tr style={{ fontWeight: 'bold', borderTop: '2px solid var(--border)' }}>
                  ...
                  <td style={{ textAlign: 'right' }}>{money(result.matrixTotals['91-120'], currency)}</td>
                  <td style={{ textAlign: 'right' }}>{money(result.matrixTotals['120+'], currency)}</td>
                  <td>100% Cartera</td>
                </tr>
              </tfoot>
            </table>
      ...
      {/* Detalle de Predicciones por Factura Abierta */}
      <section style={{ marginTop: '2rem' }}>
        ...
        >
          ...
          >
            {showInvoiceDetails ? 'Ocultar facturas' : 'Mostrar desglose por factura'}
          </button>
        </div>
                ...
                {result.predictions.map((p) => {
                  ...
                  return (
                    <tr key={p.record.id}>
                      ...
                      <td>
                        <span style={{ fontWeight: perspective === 'p50' && !asemStress ? 700 : 400 }}>
                          {p.p50ExpectedDate}
                        </span>
                      </td>
                      <td>
                        <span style={{ fontWeight: perspective === 'p80' && !asemStress ? 700 : 400 }}>
                          {p.p80ExpectedDate}
                        </span>
                      </td>
                      <td>
                        <span style={{ color: '#d97706', fontWeight: asemStress ? 700 : 400 }}>
                          {p.asemExpectedDate}
                        </span>
                      </td>
                      <td>
                        <span className={`badge ${delayVsNaive <= 0 ? 'green' : delayVsNaive <= 20 ? 'amber' : 'red'}`}>
                          {delayVsNaive > 0 ? `+${delayVsNaive} días` : `${delayVsNaive} días`}
                        </span>
                      </td>
                      <td>
      ...
      <details style={{ marginTop: '1.5rem' }}>
        ...
        <div style={{ marginTop: '0.75rem', lineHeight: 1.6, fontSize: '0.9rem' }}>
          <p>
            <strong>La ilusión de "Net 30":</strong> Los sistemas contables convencionales asumen ingenuamente que si una factura se emite a 30 días, el dinero ingresará en el día 30. En la realidad de los negocios en México y Latinoamérica, cada cliente posee un patrón de pago propio (P50: la mediana empírica en la que habitualmente liquidan sus compromisos; P80: el plazo en el que se alcanza el 80% de cobros).
          </p>
          <p>
            <strong>El Efecto ASEM de 76 días:</strong> Datos oficiales de la Asociación de Emprendedores de México (ASEM) revelan que el retraso crónico en pagos a micro, pequeñas y medianas empresas promedia <strong>76 días por encima del término pactado</strong>, lo cual destruye la liquidez operativa si no se modela preventivamente. El toggle de Estrés ASEM te permite anticipar este choque de flujo antes de contraer compromisos de pago a proveedores o inventario.
          </p>
          <p className="muted">
            Los percentiles se recalculan automáticamente con base en tus observaciones reales registradas en Finance History. Los clientes sin facturas previas heredan la distribución ponderada de la cartera global.
          </p>
        </div>
      </details>
    </Panel>
