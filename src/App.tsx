    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <nav className="relative mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8">
        <a className="flex items-center gap-3 font-semibold tracking-tight" href="/">
          <span className="grid size-9 place-items-center rounded-lg border border-border bg-primary text-primary-foreground">S</span>
          Samby
        </a>
        <AuthActions />
      </nav>
      ...
      <div className="relative mx-auto max-w-6xl px-5 pb-16 pt-16 sm:px-8 sm:pt-24">
        <section className="max-w-3xl">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-secondary">Ready for the first feature</p>
        ...
        <section className="mt-12 grid gap-3 md:grid-cols-3">
          {capabilities.map(([title, copy]) => (
            <article className="rounded-lg border border-border bg-card p-5 text-card-foreground" key={title}>
